...
const Navbar = () => {
  ...
  // ---- Refs pour le sliding pill ----
  const navLinksRef = useRef(null);
  const pillRef = useRef(null);

  // ---- Helper pour détecter le lien actif ----
  const isActive = (path) => {
    if (path === prefix) {
  ...
  // ---- Sliding pill : positionner sur le lien actif au montage / changement de route ----
  useEffect(() => {
  ...
  return (
    <>
      <header
        className={`navbar-header${visible ? "" : " navbar-header--hidden"}`}
      >
        <nav className="navbar-inner container">
          ...
          {/* Desktop nav avec sliding pill */}
          <ul
            className="navbar-links"
            ref={navLinksRef}
            onMouseLeave={handleNavMouseLeave}
          >
            {/* Pill — fond qui glisse */}
            <div className="navbar-links__pill" ref={pillRef} aria-hidden="true" />

            <li>
              <Link
                to={`${prefix}/abonnements`}
                className={`navbar-link${isActive(`${prefix}/abonnements`) ? " navbar-link--active" : ""}`}
            ...
            <li>
              <Link
                to={`${prefix}/aide`}
                className={`navbar-link${isActive(`${prefix}/aide`) ? " navbar-link--active" : ""}`}
                onMouseEnter={handleLinkMouseEnter}
              >
                {t("navbar.aide_faq")}
              </Link>
          ...
          <div className="navbar-actions">
            {IS_LOGGED_IN ? (
                  ...
                  <div className="navbar-profile__dropdown">
                    <div className="navbar-profile__header">
                      ...
                      <div className="navbar-profile__info">
                        ...
                        <span className="navbar-profile__email">
                          {mockUser.email}
                        </span>
                      </div>
                    </div>
              ...
              <>
                <Link
                  to={`${prefix}/auth/login`}
                  className="navbar-btn-connexion"
                >
                  {t("navbar.connexion")}
                </Link>
                <Link
                  to={`${prefix}/auth/register`}
                  className="navbar-btn-register btn-primary"
                >
        ...
        >
          <ul className="navbar-mobile__links">
            <li>
              <Link
                to={`${prefix}/abonnements`}
                className={`navbar-mobile__link${isActive(`${prefix}/abonnements`) ? " navbar-mobile__link--active" : ""}`}
            ...
            <li>
              <Link
                to={`${prefix}/aide`}
                className={`navbar-mobile__link${isActive(`${prefix}/aide`) ? " navbar-mobile__link--active" : ""}`}
                onClick={() => setMenuOpen(false)}
              >
                {t("navbar.aide_faq")}
              </Link>
      ...
      {logoutModal && (
        <div
          className="navbar-modal__overlay"
        ...
        >
          ...
          >
            ...
            <div className="navbar-modal__actions">
              <button
                className="navbar-modal__cancel"
                onClick={() => setLogoutModal(false)}
              >
                {t("navbar.logout_cancel")}
              </button>
              <button
                className="navbar-modal__confirm"
